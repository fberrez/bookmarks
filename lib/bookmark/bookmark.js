...
import Flickr from './provider/flickr/flickr.js';
import Vimeo from './provider/vimeo/vimeo.js';
   ...
   */
	static async getByID(id) {
		...
		try {
			...
			return throwRequestFailed(bookmark.type, bookmark.url, err);
		}
	...
	/**
	 * Replaces the keywords of the bookmark corresponding to the given id.
	 * @param {string} id bookmark object id.
	 * @param {string[]} keywords new bookmark keywords.
	 */
	static async update(id, keywords) {
		const Model = mongo.getConn().model('bookmark', bookmarkModel);
		return Model.updateOne({_id: id}, {$set: {keywords}});
	}

	/**
	 * Deletes the bookmark corresponding to the given id.
	 * @param {string} id bookmark object id.
	 */
	static async del(id) {
		const Model = mongo.getConn().model('bookmark', bookmarkModel);
